...
function save() {
  ...
  if (editing.value) setStudents(items => items.map(student => student.id === editing.value ? { ...student, ...form.value, ...identifiers } : student))
  else setStudents(items => [...items, { id: `st${Date.now()}`, ...form.value, ...identifiers, enrolledAt: new Date().toISOString().split('T')[0] }])
  modal.value = false
}
